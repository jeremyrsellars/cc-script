...
knitr::opts_chunk$set(fig.path = "2018-07-images/")
#knitr::opts_chunk$set(dev="svg") # This can be used to generate SVG files, but not in Word format.
...
dt <- median_response_time_for_all_ops
end_to_end_median_response_time_for_all_ops <-
  aggregate(dt[,"Seconds"], by=list(dt$Browser, dt$Time), FUN=sum)
names(end_to_end_median_response_time_for_all_ops) <- c("Browser", "Time", "Seconds")
end_to_end_median_response_time_for_all_ops$OperationType="EndToEnd"
end_to_end_median_response_time_for_all_ops$Operation="EndToEnd"
#response_time_plot(end_to_end_median_response_time_for_all_ops)

response_time_area_plot <- function(agg, agg_fn_desc="Median"){
  ggplot(agg, aes(x=Time, y=Seconds, color=Operation)) +
#    geom_line() + 
    geom_area(aes(fill=Operation)) +
    geom_vline(xintercept = c(8,17), linetype = "dashed", colour = "red") +
    facet_wrap("Browser") +
    scale_x_continuous(labels=function(s)paste0(s, ":00"), breaks=c(0, 8, 16, 24)) +
    ggtitle(paste0(agg_fn_desc, " response time for 3 patients by hour of day"))
}

#response_time_area_plot(median_response_time_for_all_ops)

```

This second plot adds a column to show how much slower IE is than Chrome: `(Chrome.Median - IE.Median)`

```{r _3_ResponseTime_IE_Slower, fig.width=12}
...
# Appendix C - Search Response Time Comparison Between Studies

### Search by PIN Operation

This is a best guess on comparing the performance of the Search Patient step (by PIN in the header search box).  It compares the data gathered from Chrome automation from the two studies (in November 2017, and July, 2018).
...
patient_2017 <- patient_2017[which(patient_2017$Open < 750),c("StartSearch", "Search")] # Remove probable outlier
names(patient_2017) <- c("Instant", "SearchSeconds")


patient_all <- rbind(
  patient[which(patient$Browser == "Chrome"), c("Instant", "SearchSeconds")],
  patient_2017
)
patient_all$Year <- year(patient_all$Instant)
patient_all$Hour <- hour(patient_all$Instant)
patient_all$Browser <- "Chrome"

response_time_plot(aggregate_response_time(patient_all, "SearchSeconds", "Year", mean)) +
  ggtitle("Mean response time by hour of day")

```

**Note: ** The patients used for the study are different.  In the "Browser Comparison by Operation" of the report, we do see consistent differences in the amount of time to search for different patients.

#### Findings

Generally, it appears that the search result timings have improved.  This is a small fraction of the overall workflow.

#### Search Timing Results from 2017

`r kable(summary(patient_2017))`

#### Search Timing Results from 2018

`r kable(summary(patient[which(patient$Browser == "Chrome"), c("Instant", "SearchSeconds")]))`

### Open Patient Operation

Data collected between 2017 and 2018 for opening a patient is too different to be relevant because of the differences in data retrieved.  No useful comparison can be made.

Perhaps a comparison could be made if the response time was normalized (divided by number of labs retrieved), a comparison could be useful.  No such data is available from the 2017 study.

Even if the additional data were available from 2017, this would still only represent a particular type of usage (since it retrieves data, but not data for other panels of the user interface).

#### Findings

It is noteworthy that the "Max" time to open was significantly slower in 2017.  This is further evidence that the Clinical Connect "long tail" has improved dramatically. 

#### Open "Real" Patient Timing Results from 2017

```{r}
rp_2017 <- rbind(
  open_006("..\\data\\AllRegionalEncounters_180d_0.0.6\\rp1.csv"),
  open_006("..\\data\\AllRegionalEncounters_180d_0.0.6\\part2\\rp1.csv"))
rp_2017 <- rp_2017[which(rp_2017$Open < 750),c("StartOpen", "Open")] # Remove probable outlier
names(rp_2017) <- c("Instant", "OpenSeconds")
kable(summary(rp_2017))
```

#### Open Patient Timing Results from 2018

```{r}
kable(summary(patient[which(patient$Browser == "Chrome"), c("Instant", "OpenSeconds")]))
```