...
aggregate_response_time <- function(dt, field, subfield=NULL, fn=median){
  ...
  if(useSub)
    agg$Operation <- paste0(op, floor(agg[,subfield]/1000000))
  else
    agg$Operation <- op
...
```{r}
all_response_times <-
  rbind(data.frame(list(Browser=login[,"Browser"], Seconds=login[,"LoginSeconds"])),
        data.frame(list(Browser=patient[,"Browser"], Seconds=patient[,"SearchSeconds"])),
        data.frame(list(Browser=patient[,"Browser"], Seconds=patient[,"OpenSeconds"])))
